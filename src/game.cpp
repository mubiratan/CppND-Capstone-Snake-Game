      ...
      random_h(0, static_cast<int>(grid_height))
{
  ...
  _numberOfSlownDown = config.getNumberOfSlownDown();
  ...
  _slowDown = std::shared_ptr<SlowDown>(new SlowDown());
               ...
               std::size_t target_frame_duration)
{
  ...
  while (running)
  {
    ...
    renderer.Render(snake, food, _obstacles, _slowDown);
...
void Game::PlaceSlowDown()
{
  ...
  {
    /// Return if we meet the size requiremens.
    if (_numberOfSlownDown == _slowDown->count())
    {
      return;
    }
    ...
    auto boosterExistsAtThisCell = _slowDown->SlowDownCell(Coordinate(x,y));
    if (!snake.SnakeCell(x, y) && !foodExistsAtThisCell && !boosterExistsAtThisCell)
    {
      _slowDown->addCoordinate(Coordinate(x,y));
    }
  }
}
